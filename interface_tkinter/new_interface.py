...
def onclick1(sentence,i,sentences ,event):
    ...
    Label(newWindow,text =sentence).pack()
    
    Label(newWindow,text = "is contradictory ").pack()
    Label(newWindow,text = "with").pack()
    d = str(sentences.iloc[i,2])
    for i in d.split(','): 
        Label(newWindow,text =sentences.iloc[int(i),0]).pack()
        
def update_text(sentences):
    
    txt_2.insert(INSERT, "\n")
    txt_2.insert(INSERT, "\n")
    txt_2.insert(INSERT,"Texte modifié \n\n")
...
def openNewWindow():
        ...
        for j, col in enumerate(column_names):
            text = Text(newWindow, width=16, height=1, bg = "#9BC2E6")
            text.grid(row=i,column=j)
            text.insert(INSERT, col)
        # adding all the other rows into the grid
        for i in range(n_rows):
            for j in range(n_cols):
                text = Text(newWindow, width=16, height=1)
...
sous_titre_droit = _canvas.create_text(1250, 125, text="Texte modifié :", fill="black", font=('Helvetica 24 bold underline'))
